...
{
	return pci_epc_unmap_addr(epf->epc, epf->func_no, epf->vfunc_no, peer_mem->aper);
}
...
set_outbound_translation(struct pci_epf *epf, struct pci_aper_t *peer_mem,
			 u64 peer_iova)
{
	return pci_epc_map_addr(epf->epc, epf->func_no, epf->vfunc_no, peer_mem->aper,
...
nvscic2c_pcie_epf_bind(struct pci_epf *epf)
{
	...
	if (!epf_ctx->notifier_registered) {
		...
		epf->nb.notifier_call = nvscic2c_pcie_epf_block_notifier;
		pci_epc_register_block_notifier(epf->epc, &epf->nb);
